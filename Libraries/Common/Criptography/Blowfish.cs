...
namespace Myframework.Libraries.Common.Criptography
{
    ...
    public class BlowFish
    {
        ...
        public BlowFish(string cipherKey)
        {
            byte[] keyToBytes = Encoding.ASCII.GetBytes(cipherKey);
        ...
        public string Encrypt_CBC(string value)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(Encoding.ASCII.GetBytes(value)));
        }
        ...
        public string Decrypt_CBC(string value)
        {
            IV = HexToByte(value.Substring(0, 16));
            return Encoding.ASCII.GetString(Decrypt_CBC(HexToByte(value.Substring(16)))).Replace("\0", "");
        }
        ...
        /// <returns>Texto simples</returns>
        public byte[] Decrypt_CBC(byte[] value) => Crypt_CBC(value, true);
        ...
        /// <returns></returns>
        public string Decrypt_ECB_From_Base_64(string value) => Encoding.ASCII.GetString(Decrypt_ECB(Convert.FromBase64String(value))).Replace("\0", "");
        ...
        public byte[] Encrypt_CBC(byte[] pvalue) => Crypt_CBC(pvalue, false);
        ...
        public string Encrypt_ECB_To_Hex(string value) => ByteToHex(Encrypt_ECB(Encoding.ASCII.GetBytes(value)));
        ...
        public string Encrypt_ECB_To_Base_64(string value) => Convert.ToBase64String(Encrypt_ECB(Encoding.ASCII.GetBytes(value)));
        ...
        public string Encrypt_ECB_To_String(string value) => Encoding.ASCII.GetString(Encrypt_ECB(Encoding.ASCII.GetBytes(value)));
        ...
        public string Decrypt_ECB(string value) => Encoding.ASCII.GetString(Decrypt_ECB(HexToByte(value))).Replace("\0", "");
        ...
        public byte[] Encrypt_ECB(byte[] value) => Crypt_ECB(value, false);
        ...
        public byte[] Decrypt_ECB(byte[] value) => Crypt_ECB(value, true);
        ...
        /// <param name="value">Texto simples para criptografar</param>
        /// <returns>Texto criptografado</returns>
        public string Encrypt_CTR(string value)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Crypt_CTR(Encoding.ASCII.GetBytes(value), 2));
        }
        ...
        public string Decrypt_CTR(string value)
        {
            IV = HexToByte(value.Substring(0, 16));
            return Encoding.ASCII.GetString(Crypt_CTR(HexToByte(value.Substring(16)), 2)).Replace("\0", "");
        }
        ...
        /// <returns></returns>
        public byte[] Crypt_CTR(byte[] value, int numThreads)
        {
            if (!IVSet)
            {
            ...
            int paddedLen = (value.Length % 8 == 0 ? value.Length : value.Length + 8 - (value.Length % 8));
            byte[] plainText = new byte[paddedLen];
            Buffer.BlockCopy(value, 0, plainText, 0, value.Length);
            byte[] block = new byte[8];
            for (int i = 0; i < plainText.Length; i += 8)
            {
                for (int x = 0; x < 8; x++)
